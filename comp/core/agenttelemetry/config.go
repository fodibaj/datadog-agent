...
// AgentTelemetryConfig is the top-level config for agent telemetry
...
// AgentMetricsConfig specifies agent telemetry metrics payloads to be generated and emitted
...
// Metrics is a list of metric selecting subset of telemetry.Gather() metrics to be included in agent
type MetricConfig struct {
	Name string `yaml:"name"` // required
}

// Status is a single agent telemetry status payload
		...
		if p.Metric.Exclude != nil {
			...
			for _, t := range p.Metric.Exclude.Tags {
				...
				if v, ok := p.excludeTags[tv[0]]; ok {
					...
					if _, ok := v.(struct{}); ok {
						// do nothing, we already have a value-less tag
					} else {
...
func compileAgentTelemetrySchedules(cfg *AgentTelemetryConfig) error {
...
func compileAgentTelemetryConfig(cfg *AgentTelemetryConfig) error {
	for i := 0; i < len(cfg.Profiles); i++ {
...
func parseConfig(cfg config.Component) (*AgentTelemetryConfig, error) {
	// Is it enabled?
	...
	// Parse agent telemetry config
	var atCfg AgentTelemetryConfig
	...
	err = compileAgentTelemetryConfig(&atCfg)
	if err != nil {
		return nil, err
	}
