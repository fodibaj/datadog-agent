...
type senderImpl struct {
	...
	agentTelemetryPayloadTemplate AgentTelemetryPayload
...
// Should be top level object in the payload but currently tucked into specific payloads
...
// AgentMetricsPayload exported so it can be turned into json
...
func newSenderImpl(agentCfg config.Component, log log.Component, client client) (sender, error) {
	// Form Agent Telemetry endpoint
	endpoint := utils.GetMainEndpoint(agentCfg, telemetryEndpointPrefix, "dd_url")
...
func (s *senderImpl) flushSession(ss *senderSession) error {
	...
	s.log.Infof("Flushing Agent Telemetery session with %d payloads", len(ss.payloads))
	...
	payload := s.agentTelemetryPayloadTemplate
	...
	s.addHeaders(req, payload.RequestType, s.agentCfg.GetString("api_key"), strconv.Itoa(len(reqBody)))
	...
	// Log return status
	s.log.Infof("Telemetery request response status: %s, status code: %d", resp.Status, resp.StatusCode)
...
func (s *senderImpl) sendAgentMetricPayloads(session *senderSession, metrics []*agentmetric) error {
	...
	// We will batch multipls metrics payloads into single "batch" payload type
...
func (s *senderImpl) addHeaders(req *http.Request, requesttype, apikey, bodylen string) {
	...
	// Not clear how to aquire that. Appears that EVP adds it automatically
