...
// APMInstrumentation is the main handler for the APM instrumentation mutating webhook endpoints
type APMInstrumentation struct {
	// filter is used to filter the pods to instrument
	filter *containers.Filter
}

// NewAPMInstrumentation returns a new instance of APMInstrumentation
func NewAPMInstrumentation() (*APMInstrumentation, error) {
...
func (apmInstrumentation *APMInstrumentation) InjectAutoInstrumentation(rawPod []byte, _ string, ns string, _ *authenticationv1.UserInfo, dc dynamic.Interface, _ k8s.Interface) ([]byte, error) {
	return mutate(rawPod, ns, apmInstrumentation.injectAutoInstrumentation, dc)
}
...
func (apmInstrumentation *APMInstrumentation) injectAutoInstrumentation(pod *corev1.Pod, _ string, _ dynamic.Interface) error {
	if pod == nil {
		return errors.New("cannot inject lib into nil pod")
	}
	injectApmTelemetryConfig(pod)

	if apmInstrumentation.isEnabled(pod.Namespace) {
	...
	libsToInject, autoDetected := apmInstrumentation.extractLibInfo(pod, containerRegistry)
	...
	return apmInstrumentation.injectAutoInstruConfig(pod, libsToInject, autoDetected, injectionType)
}
...
func (apmInstrumentation *APMInstrumentation) extractLibInfo(pod *corev1.Pod, containerRegistry string) ([]libInfo, bool) {
	...
	if apmInstrumentation.shouldInject(pod) {
...
func (apmInstrumentation *APMInstrumentation) isEnabled(namespace string) bool {
	...
	return !apmInstrumentation.filter.IsExcluded(nil, "", "", namespace)
}

// shouldInject returns true if Admission Controller should inject standard tags, APM configs and APM libraries
func (apmInstrumentation *APMInstrumentation) shouldInject(pod *corev1.Pod) bool {
	...
	return apmInstrumentation.isEnabled(pod.Namespace) || config.Datadog.GetBool("admission_controller.mutate_unlabelled")
}

func (apmInstrumentation *APMInstrumentation) injectAutoInstruConfig(pod *corev1.Pod, libsToInject []libInfo, autoDetected bool, injectionType string) error {
	var lastError error
