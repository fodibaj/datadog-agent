...
var ebpfConntrackerCORECreator func(cfg *config.Config, bpfTelemetry *ebpftelemetry.EBPFTelemetry) (*manager.Manager, error) = getCOREConntracker
var ebpfConntrackerRCCreator func(cfg *config.Config, bpfTelemetry *ebpftelemetry.EBPFTelemetry) (*manager.Manager, error) = getRCConntracker
var ebpfConntrackerPrebuiltCreator func(cfg *config.Config, bpfTelemetry *ebpftelemetry.EBPFTelemetry) (*manager.Manager, error) = getPrebuiltConntracker
...
func NewEBPFConntracker(cfg *config.Config, bpfTelemetry *ebpftelemetry.EBPFTelemetry) (netlink.Conntracker, error) {
	...
	if cfg.EnableCORE {
		m, err = ebpfConntrackerCORECreator(cfg, bpfTelemetry)
		if err != nil {
	...
	if m == nil && allowRC {
		m, err = ebpfConntrackerRCCreator(cfg, bpfTelemetry)
		if err != nil {
	...
	var isPrebuilt bool
	if m == nil {
		m, err = ebpfConntrackerPrebuiltCreator(cfg, bpfTelemetry)
		if err != nil {
...
func getManager(cfg *config.Config, buf io.ReaderAt, opts manager.Options, bpfTelemetry *ebpftelemetry.EBPFTelemetry) (*manager.Manager, error) {
...
func getPrebuiltConntracker(cfg *config.Config, bpfTelemetry *ebpftelemetry.EBPFTelemetry) (*manager.Manager, error) {
	...
	return getManager(cfg, buf, opts, bpfTelemetry)
}

func getRCConntracker(cfg *config.Config, bpfTelemetry *ebpftelemetry.EBPFTelemetry) (*manager.Manager, error) {
	...
	return getManager(cfg, buf, manager.Options{}, bpfTelemetry)
}

func getCOREConntracker(cfg *config.Config, bpfTelemetry *ebpftelemetry.EBPFTelemetry) (*manager.Manager, error) {
	var m *manager.Manager
	var err error
	err = ddebpf.LoadCOREAsset(netebpf.ModuleFileName("conntrack", cfg.BPFDebug), func(ar bytecode.AssetReader, o manager.Options) error {
		...
		m, err = getManager(cfg, ar, o, bpfTelemetry)
		return err
	})
	return m, err
}
