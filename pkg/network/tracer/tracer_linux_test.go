...
func TestEbpfConntrackerFallback(t *testing.T) {
	...
	for _, te := range dtests {
		t.Run("", func(t *testing.T) {
			...
			ebpfConntrackerCORECreator = getCOREConntracker
			if te.prebuiltError {
				ebpfConntrackerPrebuiltCreator = func(_ *config.Config, _ *ebpftelemetry.EBPFTelemetry) (*manager.Manager, error) {
					return nil, assert.AnError
				}
			}
			if te.rcError {
				ebpfConntrackerRCCreator = func(_ *config.Config, _ *ebpftelemetry.EBPFTelemetry) (*manager.Manager, error) {
					return nil, assert.AnError
				}
			}
			if te.coreError {
				ebpfConntrackerCORECreator = func(_ *config.Config, _ *ebpftelemetry.EBPFTelemetry) (*manager.Manager, error) {
					return nil, assert.AnError
				}
			}
